...
let id_check =(req,res,next)=>{
  ...
  let check_id_query = "SELECT product_id from st_ready_product_stock WHERE product_id=?";
  conn.query(check_id_query,[id],(err,results,fields)=>{
    if(err){
      return res.json({ message:"internal error occured ",error:err})
    }
    if(results.length==0){
      return res.json({ message:" ready_product of provided id doesn't exit " });
    }
    next();
  })
}
...
router.patch("/:id", id_check,(req, res) => {
  ...
  let values = Object.values(ready_product);
  values.push(id);
  let update_query = "UPDATE st_ready_product_stock SET "+keys.map((key)=>`${key}=?`).join(",")+" WHERE product_id=? ";
  conn.query(update_query,values,(err,results,fields)=>{
    if(err){
      return res.status(456).json({message:"some error occured ",error:err});
    }
    return res.status(200).json({ message: " ready_product of provided id updated  " });
  })
});
...
router.delete("/:id",id_check,(req, res) => {
  ...
  if (!id) {
    ...
    let sql = "DELETE from st_ready_product_stock WHERE product_id=?";
      conn.query(sql, [id], (err, results, fields) => {
        console.log(results);
          return res.status(200).json({ message: " ready_product of provided id deleted  " });
      });   
});
...
router.get('/subs/:id',(req,res)=>{
    ...
    conn.query(sql, (err, results, fields) => {
      if (err) {
        ...
        ready_products=ready_products.filter(f=>f['product_category_id']===Number(id));
        return res.json({ready_products});
      }
    });
})  
