...
router.get('/ready_product/:id', (req, res) => {
    const { id } = req.params;
    let sql = "select * from st_manufacturing_raw_qty_info a left join st_raw_material_stock b on a.raw_material_id = b.product_id";
    conn.query(sql, [id], (err, results, fields) => {
        if (err) {
            console.log(err);
            return res.status(404).send("manufacturing_raw_qty not found");
            ...
            let manufacturing_raw_qty = results.map(row => ({ ...row }));
            let data = manufacturing_raw_qty.filter(t => t.ready_product_id === Number(id));
